...
class AgentSimulator:
    ...
    async def discover_fact(
        self, content: str, scope: str, discovery_time: float
    ) -> dict[str, Any]:
...
async def test_compare_scenarios(engine: EngramEngine):
    ...
    await baseline_agent_a.discover_fact("JWT tokens expire after 3600 seconds", "auth/jwt", 15.0)
    await baseline_agent_a.discover_fact(
        "Auth service rate-limits to 1000 req/s per IP", "auth/ratelimit", 20.0
    )
    await baseline_agent_a.discover_fact("Refresh tokens are valid for 30 days", "auth/jwt", 12.0)

    await baseline_agent_b.discover_fact("JWT tokens expire after 3600 seconds", "auth/jwt", 15.0)
    await baseline_agent_b.discover_fact(
        "Auth service rate-limits to 1000 req/s per IP", "auth/ratelimit", 20.0
    )
    await baseline_agent_b.discover_fact("Refresh tokens are valid for 30 days", "auth/jwt", 12.0)
